...
// --- Universal Accessibility & Security Enhancements ---
...
// Accessibility: FAB keyboard support
document.addEventListener('DOMContentLoaded', function() {
    const fab = document.querySelector('.fab');
    if (fab) {
        fab.setAttribute('tabindex', '0');
        fab.setAttribute('aria-label', 'Contact options');
        fab.setAttribute('title', 'Contact options');
        fab.onkeydown = function(e) {
            if (e.key === "Enter" || e.key === " ") {
                if (typeof toggleFabOptions === "function") toggleFabOptions();
            }
        };
    }
    ...
    // Accessibility: Product card buttons
    document.querySelectorAll('.product-card .btn').forEach(btn => {
        btn.setAttribute('aria-label', 'View details');
        btn.setAttribute('title', 'View details');
    });

    // Accessibility: Contact form fields and button (contact.html)
    const contactForm = document.getElementById('contactForm');
    if (contactForm) {
        contactForm.querySelectorAll('input, textarea').forEach(field => {
            if (field.placeholder) {
                field.setAttribute('aria-label', field.placeholder);
                field.setAttribute('title', field.placeholder);
            }
        });
...
// Cart count updater (universal)
function updateCartCount() {
    let cart = JSON.parse(localStorage.getItem('cart') || '[]');
    const cartCountElem = document.getElementById('cart-count');
    if (cartCountElem) {
        cartCountElem.textContent = cart.reduce((sum, i) => sum + (i.qty || 1), 0);
    }
}
